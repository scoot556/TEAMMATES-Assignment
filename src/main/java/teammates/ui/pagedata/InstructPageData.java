...
public class InstructPageData {
    ...
    public InstructPageData(InstructAccountAttributes account, String sessionToken) {
        this(account, null, sessionToken);
    }

    public InstructPageData(InstructAccountAttributes account, InstructorAttributes instructor, String sessionToken) {
        this.account = account;
        this.instructor = instructor;
        this.sessionToken = sessionToken;
    }

    public InstructAccountAttributes getAccount() {
        return account;
    }
    ...
    public boolean isUnregisteredStudent() {
        return account.googleId == null || instructor != null && !instructor.isRegistered();
    }
    ...
    /**
     * Returns the status of the instructor, whether he has joined the course.
     ...
     */
   /* public String getInstructorStatus(InstructorAttributes instructor) {
        return instructor.getInstructorStatus();
    }*/

    /**
     * Returns The relative path to the instructor home page. Defaults to whether the instructor is unregistered.
     */
   /* public String getInstructorHomeLink() {
        return getInstructorHomeLink(isUnregisteredInstructor());
    }*/

    /**
     * Returns The relative path to the instructor home page. The user Id is encoded in the url as a parameter.
     */
    /*public String getInstructorHomeLink(boolean isUnregistered) {
        String link = Const.ActionURIs.INSTRUCTOR_HOME_PAGE;
        link = addUserIdToUrl(link);
        if (isUnregistered) {
            link = Url.addParamToUrl(instructor.getRegistrationUrl(), Const.ParamsNames.NEXT_URL, link);
        }
        return link;
    }*/

    /**
     * Returns The relative path to the instructor profile page. Defaults to whether the instructor is unregistered.
     */
   /*
    public String getInstructorProfileLink() {
        return getInstructorProfileLink(isUnregisteredInstructor());
    }
    */

    /**
     * Returns The relative path to the instructor profile page. The user Id is encoded in the url as a parameter.
     */
   /* public String getInstructorProfileLink(boolean isUnregistered) {
...
/*
    ...
    public String getInstructorProfilePictureLink(String studentEmail, String courseId) {
        String link = Const.ActionURIs.INSTRUCTOR_PROFILE_PICTURE;
        link = Url.addParamToUrl(link, Const.ParamsNames.INSTRUCTOR_EMAIL, studentEmail);
        link = Url.addParamToUrl(link, Const.ParamsNames.COURSE_ID, courseId);
        link = addUserIdToUrl(link);
        return link;
    }*/
    ...
    /**
     * Retrieves the link to submit the request for remind instructor
     * Appends the return url to the link.
    ...
    /**
     * Retrieves the link to submit the request to remind a particular instructor(s).
     * @param returnUrl the url to return to after submitting the request
     * @return submit link with return url appended to it
     */
   ...
   /* public InstructorFeedbackSessionActions getInstructorFeedbackSessionActions(FeedbackSessionAttributes session,
    ...
    public String getRecipientNames(Set<String> recipients, String courseId, String studentEmail, CourseRoster roster) {
        ...
        for (String recipient : recipients) {
            ...
            InstructorAttributes instructor = roster.getInstructorForEmail(recipient);
            if (recipient.equals(studentEmail)) {
                ...
                if (recipients.size() == 1) {
                    namesStringBuilder.append(instructor.name + " (" + instructor.displayedName + ", " + instructor.email + ")");
                } else {
                    namesStringBuilder.append(instructor.name);
                }
            }
            i++;
        }
