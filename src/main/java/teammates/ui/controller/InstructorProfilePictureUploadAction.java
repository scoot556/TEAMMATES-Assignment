...
public class InstructorProfilePictureUploadAction extends Action{
    ...
    protected ActionResult execute() throws EntityDoesNotExistException {
        ...
        RedirectResult r = createRedirectResult(Const.ActionURIs.INSTRUCTOR_PROFILE_PAGE);
        ...
        try {
            ...
            if (!isError) {
                ...
                statusToUser.add(new StatusMessage(Const.StatusMessages.INSTRUCTOR_PROFILE_PICTURE_SAVED,
                                                   StatusMessageColor.SUCCESS));
                r.addResponseParam(Const.ParamsNames.INSTRUCTOR_PROFILE_PHOTOEDIT, "true");
            }
    ...
    private BlobInfo extractProfilePictureKey() {
        try {
            ...
            List<BlobInfo> blobs = blobsMap.get(Const.ParamsNames.INSTRUCTOR_PROFILE_PHOTO);
            if (blobs == null || blobs.isEmpty()) {
                statusToUser.add(new StatusMessage(Const.StatusMessages.INSTRUCTOR_PROFILE_NO_PICTURE_GIVEN,
    ...
    private BlobInfo validateProfilePicture(BlobInfo profilePic) {
        if (profilePic.getSize() > Const.SystemParams.MAX_PROFILE_PIC_SIZE) {
            ...
            isError = true;
            statusToUser.add(new StatusMessage(Const.StatusMessages.INSTRUCTOR_PROFILE_PIC_TOO_LARGE,
                                               StatusMessageColor.DANGER));
            return null;
        } else if (!profilePic.getContentType().contains("image/")) {
            deletePicture(profilePic.getBlobKey());
            isError = true;
            statusToUser.add(new StatusMessage(Const.StatusMessages.INSTRUCTOR_PROFILE_NOT_A_PICTURE,
                                               StatusMessageColor.DANGER));
            return null;
        }
    ...
    private void updateStatusesForBlobstoreFailure() {
        ...
        statusToUser.clear();
        statusToUser.add(new StatusMessage(Const.StatusMessages.INSTRUCTOR_PROFILE_PIC_SERVICE_DOWN,
                                           StatusMessageColor.DANGER));
    }
